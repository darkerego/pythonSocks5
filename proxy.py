...
class SOCKS5ProxyHandler(BaseRequestHandler):
    ...
    def _recv(self, bufsize: object, failure_method: object = False, code: object = False) -> object:
    ...
    def _verify_credentials(self):
        ...
        if version != USERNAME_PASSWORD_VERSION:
            logger.error(f'USERNAME_PASSWORD_VERSION did not match')
...
if __name__ == '__main__':
    ...
    args.add_argument('-u', '--user', type=str, default=None)
    args.add_argument('-P', '--pass', dest='password', type=str, default=None)
    args.add_argument('-l', '--listen', dest='listen_ip', type=str, default='0.0.0.0')
    args.add_argument('-p', '--port', type=int, default=1080)
    args.add_argument('-b', '--bind', type=str, dest='bind_address', default=None)
    args.add_argument('-c', '--config', type=str, default=None,
                      help='Load a json config file instead of specifying on the cli.')
    args.add_argument('-d', '--dump', action='store_true', help='Dump the specified cli '
                                                                'args into json. Redirect to a file to '
                                                                'load for later use as --config.')
    ...
    if args.user and args.password:
        _options.update({'auth': (args.user, args.password)})  # default = not set
    if args.bind_address:
        _options.update({'bind_address': args.bind_address})  # default = not set
        ...
        with open(args.config, 'r') as f:
            ...
            if len(cfg.items()):
                for key, value in cfg.items():
                    if key == 'auth':
                        _options['auth'] = tuple(value)
                    else:
                        _options[key] = value
    if args.dump:
        print(json.dumps(_options))
        exit()

    # Options are completely optional
    with SOCKS5ProxyServer(_options) as server:
